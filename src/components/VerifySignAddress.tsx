...
export default function VerifySignAddress({
  ...
  const [signedMessageResult, setSignedMessageResult] =
    useState<SignedMessageResult>();
  const [signature, setSignature] = useState("");
  const [bytes, setBytes] = useState("");
  const currentAccount = useCurrentAccount();

  async function verifySignedAddress(
    streamer: string,
    address: string,
    signature: string
  ) {
                  ...
                  content: (
                    <div className="h-full flex flex-col">
                      <p className="font-bold text-xl mb-1">
                        To verify your address, click the sign message below.
                        This doesn&#39;t cost any GAS/MIST, and is done locally.
                      </p>
                      <div>
                        <label className="font-bold text-xl text-gr mb-1 block max-md:text-center">
                          SUI Identifier{" "}
                          {_signature ? (
                            <span className="text-[#FFE500]">(Verified)</span>
                          ) : (
                            <span className="text-[#05FF00]">Unverified</span>
                          )}{" "}
                          <a
                            href="https://suins.io/"
                            target="_blank"
